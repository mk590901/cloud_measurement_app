...
class Controller implements IController, IAlertResponse {
  ...
  bool _continuosMeasurement = false;
  ...
  AlertsWrapper _alertWrapper = AlertsWrapper(1);
  ...
  bool isContinuosMeasurement() {
    return _continuosMeasurement;
  }

  @override
  String getValue() {
    return _simulator.getValue();  
  }
  ...
  void start(String key) async {
    ...
    _listeners.listeners().forEach((key, updater) {
      updater?.update(getLastValue(), true, MeasurementError(_lastError), getLastTime());
    });
...
//  Alert
  ...
  void updateAlertWrapper(int capacity) {
    _alertWrapper = AlertsWrapper(capacity);
  }
