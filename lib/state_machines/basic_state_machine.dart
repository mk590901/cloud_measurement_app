...
//  Constructor
  ...
  int dispatch(Event event) {
    ...
    if (transaction == null) {
      print ("Failed to get transaction for $machine [${getStateName(_currentState)}], event->${event}");
    ...
    if (machine != 'RefreshingStateMachine') {
      print('dispatch $machine|${event.runtimeType}: $prevState -> $currState');
    }
